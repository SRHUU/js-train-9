...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((ind) => ind.name === name);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArrayString = slice.join(" ");
...
function getPositiveArrayValues(arr) {
  ...
  const arrValues = Array.from(iter);
  const positiveArrayValues = arrValues.filter((element) => element > 0);
...
function sumAndPrint(arr) {
  ...
  const sum = arr.reduce(
    (accumulator, curentValue) => curentValue + accumulator,
    0
  );
  arr.forEach((element) => {
    console.log(element);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const doubleElement = array.map((element) => element ** 2);
  const filteredSquaredArray = doubleElement.filter(
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const arrSmall = arr.slice(-n);
  const sum = arrSmall.reduceRight(
    (accumulator, curentValue) => accumulator + curentValue,
    0
  );
  arrSmall.forEach((element) => {
    console.log(element);
  });
  return sum;
}
...
function wordLengths(arr) {
  ...
  const lengthArray = arr.map((element) => element.length);
  return lengthArray;
}
...
function expandArrayByN(arr, n) {
  ...
  if (typeof n !== "number") {
    return "Другий вхідний параметр має бути числом";
  }
  const expandArray = arr.flatMap((el) => Array(n).fill(el));
  return expandArray;
}
...
function findDuplicateElements(arr) {
  ...
  const filterFn = (element, index) => arr.indexOf(element) !== index;
  const duplicateElements = arr.filter(filterFn);
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((num, value) => num + value.price, 0);
  return totalPrice;
}
